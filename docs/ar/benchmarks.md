...
هذه الأرقام reproducible development metrics وليست marketing claims. تقيس
standalone `nllclw` binary وتتجنب إخفاء العمل في external programs مثل `curl`.

## Current Snapshot

قيس محلياً على macOS arm64 (Apple M3 Max) من ReleaseSmall build. تستخدم runtime
metrics الأمر `nllclw --help` كي يقاس startup بلا provider أو network work.
...
- `878.7 KiB` هي binary value. يطبع macOS `ls -lh` هذه القيمة ك`879K`.
  بوحدات decimal تكون `899.8 KB`.
- يعرض macOS `/usr/bin/time -l` process RSS، وليس Zig heap usage. إن process صغيراً
  مثل `/usr/bin/true` يقارب مسبقاً `1.14 MiB` RSS على هذه machine، لذلك تظهر
  المقارنة المفيدة أيضاً ك"over `/usr/bin/true`".
- هذه أرقام standalone-binary. مسار HTTP/TLS موجود في Zig binary عبر
  `std.http.Client`; القياس لا يخفي العمل في system `curl`.

## Startup Formula

يتبع startup أسلوب normalization نفسه مثل `nullclaw`: استخدم CPU cycle count من
`/usr/bin/time -l`، واطرح `/usr/bin/true` process baseline، ثم normalize العمل
المتبقي إلى 0.8 GHz edge core.
...
يأتي current p50 startup number من 200 runs:
...
p50 startup وRSS over 200 runs:
