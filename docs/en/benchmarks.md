...
These numbers are intended to be reproducible development metrics, not marketing
claims. They measure the standalone `nllclw` binary and avoid hiding work in
external programs such as `curl`.

## Current Snapshot

Measured locally on macOS arm64 (Apple M3 Max) from a ReleaseSmall build.
Runtime metrics use `nllclw --help` so startup is measured without provider or
network work.
...
- `878.7 KiB` is the binary value. macOS `ls -lh` prints this as `879K`.
  In decimal units this is `899.8 KB`.
- macOS `/usr/bin/time -l` reports process RSS, not Zig heap usage. A tiny
  `/usr/bin/true` process is already about `1.14 MiB` RSS on this machine, so
  the useful comparison is also shown as "over `/usr/bin/true`".
- These are standalone-binary numbers. The HTTP/TLS path is in the Zig binary
  through `std.http.Client`; the measurement does not hide work in system
  `curl`.

## Startup Formula

Startup follows the same normalization style as `nullclaw`: use the CPU cycle
count from `/usr/bin/time -l`, subtract a `/usr/bin/true` process baseline, then
normalize the remaining process work to a 0.8 GHz edge core.
...
The current p50 startup number comes from 200 runs:
