...
Angka-angka ini dimaksudkan sebagai development metrics yang dapat direproduksi,
bukan marketing claims. Angka ini mengukur standalone binary `nllclw` dan tidak
menyembunyikan kerja di program eksternal seperti `curl`.

## Current Snapshot

Diukur secara lokal di macOS arm64 (Apple M3 Max) dari build ReleaseSmall.
Runtime metrics memakai `nllclw --help`, sehingga startup diukur tanpa kerja
provider atau network.
...
- `878.7 KiB` adalah nilai binary. macOS `ls -lh` mencetaknya sebagai `879K`.
  Dalam satuan desimal, nilainya `899.8 KB`.
- macOS `/usr/bin/time -l` melaporkan process RSS, bukan penggunaan heap Zig.
  Process kecil `/usr/bin/true` sudah sekitar `1.14 MiB` RSS di mesin ini,
  sehingga perbandingan berguna juga ditampilkan sebagai "over `/usr/bin/true`".
- Ini adalah standalone-binary numbers. Jalur HTTP/TLS berada di binary Zig
  melalui `std.http.Client`; pengukuran tidak menyembunyikan kerja di `curl`
  sistem.

## Rumus startup

Startup mengikuti gaya normalisasi yang sama dengan `nullclaw`: gunakan CPU
cycle count dari `/usr/bin/time -l`, kurangi baseline process `/usr/bin/true`,
lalu normalisasikan sisa process work ke edge core 0.8 GHz.
...
Angka p50 startup saat ini berasal dari 200 runs:
