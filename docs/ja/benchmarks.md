...
これらの numbers は reproducible development metrics を意図しており、marketing
claims ではありません。Standalone `nllclw` binary を measure し、`curl` のような
external programs に作業を隠さないようにしています。

## Current Snapshot

macOS arm64 (Apple M3 Max) 上で ReleaseSmall build から locally measured しました。
Runtime metrics は `nllclw --help` を使うため、startup は provider や network work
なしで measured されます。
...
- `878.7 KiB` は binary value です。macOS `ls -lh` はこれを `879K` と表示します。
  Decimal units では `899.8 KB` です。
- macOS `/usr/bin/time -l` は process RSS を report し、Zig heap usage ではありません。
  Tiny な `/usr/bin/true` process でもこの machine では約 `1.14 MiB` RSS なので、
  useful comparison として "over `/usr/bin/true`" も示しています。
- これは standalone-binary numbers です。HTTP/TLS path は `std.http.Client` を通じて
  Zig binary 内にあり、measurement は system `curl` に作業を隠していません。

## Startup Formula

Startup は `nullclaw` と同じ normalization style に従います: `/usr/bin/time -l`
から CPU cycle count を使い、`/usr/bin/true` process baseline を subtract し、
残りの process work を 0.8 GHz edge core に normalize します。
...
Current p50 startup number は 200 runs から得られます:
