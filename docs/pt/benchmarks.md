...
Estes números pretendem ser development metrics reprodutíveis, não marketing
claims. Eles medem o standalone binary `nllclw` e evitam esconder trabalho em
programas externos como `curl`.

## Current Snapshot

Medido localmente em macOS arm64 (Apple M3 Max) a partir de um build
ReleaseSmall. Runtime metrics usam `nllclw --help`, então startup é medido sem
trabalho de provedor ou rede.
...
- `878.7 KiB` é o valor do binário. macOS `ls -lh` imprime isso como `879K`.
  Em unidades decimais, isso é `899.8 KB`.
- macOS `/usr/bin/time -l` relata process RSS, não uso de heap Zig. Um processo
  minúsculo `/usr/bin/true` já tem cerca de `1.14 MiB` RSS nesta máquina, então
  a comparação útil também aparece como "over `/usr/bin/true`".
- Estes são standalone-binary numbers. O caminho HTTP/TLS está no binário Zig
  por `std.http.Client`; a medição não esconde trabalho no `curl` do sistema.

## Fórmula de inicialização

Startup segue o mesmo estilo de normalização que `nullclaw`: usa a contagem de
ciclos de CPU de `/usr/bin/time -l`, subtrai uma baseline de processo
`/usr/bin/true` e normaliza o trabalho restante do processo para um edge core de
0.8 GHz.
...
O número p50 atual de startup vem de 200 runs:
