...
Эти числа предназначены как воспроизводимые development metrics, а не marketing
claims. Они измеряют standalone binary `nllclw` и не скрывают работу во внешних
программах вроде `curl`.

## Current Snapshot

Измерено локально на macOS arm64 (Apple M3 Max) из ReleaseSmall build. Runtime
metrics используют `nllclw --help`, поэтому startup измеряется без работы
provider или network.
...
- `878.7 KiB` — это значение бинарника. macOS `ls -lh` печатает его как
  `879K`. В десятичных единицах это `899.8 KB`.
- macOS `/usr/bin/time -l` сообщает process RSS, а не Zig heap usage. Крошечный
  процесс `/usr/bin/true` уже занимает около `1.14 MiB` RSS на этой машине,
  поэтому полезное сравнение также показано как "over `/usr/bin/true`".
- Это standalone-binary numbers. HTTP/TLS path находится в Zig binary через
  `std.http.Client`; измерение не скрывает работу в системном `curl`.

## Формула запуска

Startup следует тому же стилю нормализации, что и `nullclaw`: взять CPU cycle
count из `/usr/bin/time -l`, вычесть baseline процесса `/usr/bin/true`, затем
нормализовать оставшуюся process work к 0.8 GHz edge core.
...
Текущее p50 startup number получено из 200 runs:
